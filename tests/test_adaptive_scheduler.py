...
class TestSuccessiveHalvingScheduler(unittest.TestCase):
    ...
    def test_initial_work_units(self):
        ...
        work_units, _ = scheduler.get_initial_work_units(
            list(self.experiment.trials.values()), self.experiment.scheduler_state
        )
        ...
        self.assertTrue(
            all(t.status == TrialStatus.ACTIVE for t in self.experiment.trials.values())
        )
    ...
    def test_pruning_logic_increasing_metric(self):
        ...
        pruned_trials = [
            t for t in self.trials.values() if t.status == TrialStatus.PRUNED
        ]
        active_trials = [
            t for t in self.trials.values() if t.status == TrialStatus.ACTIVE
        ]
    ...
    def test_pruning_logic_decreasing_metric(self):
        ...
        scheduler = SuccessiveHalvingScheduler(
            metric="loss", increasing=False, min_epochs_per_rung=1, reduction_factor=2
        )
        ...
        for i, trial in enumerate(self.trials.values()):
            ...
            trial.results["loss"] = [
                (1, 0.5 - i * 0.1)
            ]  # trial_1 and trial_2 have highest loss
