...
class TestInsightEngine(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.engine = InsightEngine(self.trials, self.primary_metric, self.higher_is_better)

    def _create_trial(self, trial_id, status=TrialStatus.ACTIVE, results=None, hparams=None):
    ...
    def test_detect_performance_plateau(self):
        ...
        # Plateaued trial
        trial1 = self._create_trial('t1', results=[(1, 0.8), (2, 0.801), (3, 0.802), (4, 0.803)])
        insight = self.engine._detect_performance_plateau(trial1)
        self.assertIsNotNone(insight)
        self.assertEqual(insight.type, "PLATEAU")

        # Improving trial
        trial2 = self._create_trial('t2', results=[(1, 0.8), (2, 0.85), (3, 0.9), (4, 0.95)])
    ...
    def test_detect_performance_crossover(self):
        ...
        trial2 = self._create_trial('t2', results=[(1, 0.82), (2, 0.83)]) # Loser
    ...
    def test_detect_hyperparameter_correlation_categorical(self):
        ...
        self._create_trial('t1', status=TrialStatus.COMPLETED, results=[(1, 0.90)], hparams={'optimizer_params': {'name': 'Adam'}})
        self._create_trial('t2', status=TrialStatus.COMPLETED, results=[(1, 0.92)], hparams={'optimizer_params': {'name': 'Adam'}})
        self._create_trial('t3', status=TrialStatus.COMPLETED, results=[(1, 0.91)], hparams={'optimizer_params': {'name': 'Adam'}})
        # Group 2: SGD optimizer (poor performance)
        self._create_trial('t4', status=TrialStatus.COMPLETED, results=[(1, 0.70)], hparams={'optimizer_params': {'name': 'SGD'}})
        self._create_trial('t5', status=TrialStatus.COMPLETED, results=[(1, 0.72)], hparams={'optimizer_params': {'name': 'SGD'}})
        self._create_trial('t6', status=TrialStatus.COMPLETED, results=[(1, 0.71)], hparams={'optimizer_params': {'name': 'SGD'}})
    ...
    def test_detect_hyperparameter_correlation_numerical(self):
        ...
        self._create_trial('t1_low', status=TrialStatus.COMPLETED, results=[(1, 0.95)], hparams={'optimizer_params': {'lr': 0.001}})
        self._create_trial('t2_low', status=TrialStatus.COMPLETED, results=[(1, 0.96)], hparams={'optimizer_params': {'lr': 0.002}})
        self._create_trial('t3_low', status=TrialStatus.COMPLETED, results=[(1, 0.94)], hparams={'optimizer_params': {'lr': 0.0015}})
        # Group 2: High LR (poor performance)
        self._create_trial('t4_high', status=TrialStatus.COMPLETED, results=[(1, 0.60)], hparams={'optimizer_params': {'lr': 0.1}})
        self._create_trial('t5_high', status=TrialStatus.COMPLETED, results=[(1, 0.62)], hparams={'optimizer_params': {'lr': 0.2}})
        self._create_trial('t6_high', status=TrialStatus.COMPLETED, results=[(1, 0.61)], hparams={'optimizer_params': {'lr': 0.15}})
        # Group 3: Medium LR (medium performance) - to ensure binning works
        self._create_trial('t7_med', status=TrialStatus.COMPLETED, results=[(1, 0.80)], hparams={'optimizer_params': {'lr': 0.01}})
        self._create_trial('t8_med', status=TrialStatus.COMPLETED, results=[(1, 0.82)], hparams={'optimizer_params': {'lr': 0.02}})
        self._create_trial('t9_med', status=TrialStatus.COMPLETED, results=[(1, 0.81)], hparams={'optimizer_params': {'lr': 0.015}})
