...
class TrialCheckpointManager:
    ...
    def save(self, trial: Trial, model: torch.nn.Module, optimizer: torch.optim.Optimizer) -> Optional[str]:
    ...
    def load(self, trial: Trial, model: torch.nn.Module, optimizer: torch.optim.Optimizer):
        ...
        try:
            ...
            logger.error(f"Failed to load checkpoint for trial {trial.id} from {trial.checkpoint_path}: {e}")
