...
class ExperimentOrchestrator:
    ...
    def handle_add_algorithm(self, payload: Dict[str, Any]) -> None:
        ...
        if self.experiment.status in [ExperimentStatus.RUNNING, ExperimentStatus.PAUSED]:
            ...
            # When adding mid-run, we don't have a UI setting for this,
            # so we use a reasonable default.
            self._generate_and_add_trials([new_algo], num_trials_per_algo=10)
    ...
    def handle_start_run(self, payload: Dict[str, Any]) -> None:
        ...
        if not self.experiment.trials:
            self.log_message.emit("INFO: No pre-existing trials found. Generating initial set.")
    ...
    def _generate_and_add_trials(self, algorithms: List[Any], num_trials_per_algo: int) -> bool:
        self.log_message.emit(f"INFO: Generating {num_trials_per_algo} trials for {len(algorithms)} algorithm(s).")
        try:
            ...
            self.log_message.emit(f"INFO: Added {len(new_trials)} new trials to the experiment.")
            return True
        except Exception as e:
    ...
    def _initialize_and_start_runtime(self, start_payload: Dict[str, Any] = None) -> None:
        ...
        try:
            ...
            num_workers = start_payload["num_workers"]
            ...
            self.log_message.emit(f"INFO: SdeRuntimeEngine initialized with {self.experiment.adaptive_policy} scheduler.")
