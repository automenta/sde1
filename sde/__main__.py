...
def main():
    ...
    adaptive_scheduler = SuccessiveHalvingScheduler(metric="accuracy", increasing=True, min_epochs_per_rung=2)
    ...
    orchestrator.trial_updated.connect(lambda data: logger.info(f"TRIAL UPDATE: {data['id']} - Status: {data['status']}"))
